...
//──────────────── FAST INPUT/OUTPUT ────────────────────
#define FASTINOUT ios::sync_with_stdio(false); cin.tie(nullptr)
...
// PRACTICE flag enables runtime asserts, tracing and brute solvers.
    ...
    #define ASSERT(cond,msg) \
        if(!(cond)){ cerr<<"ASSERT FAIL: "<<(msg)<<" @"<<__LINE__<<NL; exit(42); }
...
template<typename T> void read(vector<T>& v) { for (T& x : v) cin >> x; }
template<typename T> void printv(const vector<T>& v) { for (const T& x : v) cout << x << SPACE; cout << NL; }
