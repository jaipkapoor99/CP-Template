...
struct SegTreeLazy {
    ...
    SegTreeLazy(int n_val = 0, T_val default_val = 0, T_val neutral_v = 0, T_lazy neutral_l = 0) 
        ...
        if (n > 0) {
            ...
            t.clear(); lazy.clear(); range_len.clear();
        }
    }

    SegTreeLazy(const vector<T_val>& arr, T_val neutral_v = 0, T_lazy neutral_l = 0)
        : n(sz(arr)), neutral_val(neutral_v), neutral_lazy(neutral_l) { // Used sz
    ...
    void _build_from_array(const vector<T_val>& arr, int node_idx, int seg_l, int seg_r) {
        if (seg_l == seg_r) {
            if (seg_l < sz(arr)) t[node_idx] = arr[seg_l]; // Used sz
            return;
        }
        int mid = seg_l + (seg_r - seg_l) / 2;
    ...
    void _push(int node_idx) { // Push lazy value from node_idx to its children
    ...
    void _update_range(int node_idx, int seg_l, int seg_r, int qL, int qR, T_lazy val) {
        if (seg_l > qR || seg_r < qL) return; // Current segment out of query range
        if (qL <= seg_l && seg_r <= qR) { // Current segment completely within query range
    ...
    void update(int qL, int qR, T_lazy val) { // Public interface for 0-indexed [qL, qR]
    ...
    T_val _query_range(int node_idx, int seg_l, int seg_r, int qL, int qR) {
        if (seg_l > qR || seg_r < qL) return neutral_val; // Current segment out of query range
        if (qL <= seg_l && seg_r <= qR) { // Current segment completely within query range
            return t[node_idx];
        }
    ...
    T_val query(int qL, int qR) { // Public interface for 0-indexed [qL, qR]
...
struct FenwickTree {
    ...
    FenwickTree(const vector<T>& arr) : n(sz(arr)) { // Used sz
    ...
    void update(int idx, T delta) {
        ...
        for (++idx; idx <= n; idx += idx & -idx) { // Convert to 1-indexed for BIT logic
            bit[idx] += delta;
        }
    }
    ...
    T query(int idx) {
        ...
        T sum = 0;
        for (++idx; idx > 0; idx -= idx & -idx) { // Convert to 1-indexed for BIT logic
            sum += bit[idx];
        }
        return sum;
    }
...
inline vector<ll> dijkstra(int s, int n, const vector<vector<pii>>& adj) {
    ...
    while (!pq.empty()) {
        ...
        forV(adj[u]) { // Used forV macro, and edge.fi, edge.se implicitly use fi, se
            int v = e.fi;
            ll weight = e.se;
...
inline vector<ll> bfs01(int s, int n, const vector<vector<pii>>& adj) {
    ...
    while(!q_bfs.empty()) {
        ...
        forV(adj[u]) { // Used forV macro, and e.fi, e.se implicitly use fi, se
            int v = e.fi;
            int weight = e.se;
...
struct LCA {
    ...
    void add_edge(int u, int v) {
        ...
        adj[u].pb(v); // Used pb
        adj[v].pb(v); // Used pb. Corrected: should be adj[v].pb(u)
    }
    ...
    void _dfs_lca(int v, int p, int d) {
        ...
        f(j, 1, MAX_LCA_LG) { // Used f macro
            up[v][j] = up[up[v][j-1]][j-1];
        }

        forV(adj[v]) { // Used forV macro
            if (e != p) {
                _dfs_lca(e, v, d + 1);
            }
        }
    ...
    int get_kth_ancestor(int u, int k) {
        ...
        if (k < 0 || k > depth[u]) return -1; // Or handle as error/specific value
        
        rf(j, MAX_LCA_LG, 0) { // Used rf macro
            if ((k >> j) & 1) {
                u = up[u][j];
            }
        }
...
struct StringHashing {
    ...
    StringHashing(const string& s, ll b1 = 0, ll b2 = 0) : s_hashed(s), n(sz(s)) { // Used sz
        if (n == 0) return;
    ...
    pll get_hash(int l, int r) {
        if (l > r || l < 0 || r >= n) return {0,0}; // Invalid range
        if (n == 0) return {0,0};
...
inline void precompute_factorials() { // Explicitly inline, though templates in headers are often implicitly so
    ...
    if (MAX_N_COMB == 0) return; // Should not happen with current const value but good check

    fact[0] = 1;
    invFact[0] = 1;
    ModularOps<Mod> M_ops; // Local ModularOps instance for this function call
    f(i, 1, MAX_N_COMB) { // Used f macro
        // Safety check, though fact/invFact should be sized correctly by their definition.
        ASSERT(i < static_cast<int>(fact.size()) && i < static_cast<int>(invFact.size()), "Factorial precomputation index out of bounds.");
...
struct Sieve {
    ...
    Sieve(int n = 1000000) : max_val(n) {
        ...
        cf(i, 2, n) { // Used cf macro
            ...
            forV(primes) { // Used forV macro
                if (e > spf[i] || (ll)i * e > n) { // e for element in primes
    ...
    vpll get_prime_factorization(int k) {
        ...
        vpll factors;
        if (k == 1) return factors; // Or return {{1,1}} depending on convention
        while (k > 1) {
            int p = spf[k];
            int count = 0;
            while (k % p == 0) {
            ...
            factors.pb({p, count}); // Used pb and implicitly fi/se for pair construction if it were explicit
        }
        return factors;
    }
    ...
    vll get_distinct_prime_factors(int k) {
        ...
        if (k == 1) return distinct_factors;
        while (k > 1) {
            int p = spf[k];
...
inline bool isDivisible(T1 n, T2 divisor)
{
    if (divisor == 0)
    {
        return false;
...
struct Matrix {
    ...
    Matrix operator*(const Matrix& other) const {
        ...
        f(i, 0, rows) { // Used f macro
            f(j, 0, other.cols) { // Used f macro
                T sum = 0;
                f(k, 0, cols) { // Used f macro
...
struct DinicMaxFlow {
    ...
    void add_edge(int u, int v, ll cap) {
        ...
        adj[u].pb({v, cap, static_cast<int>(sz(adj[v]))});
        adj[v].pb({u, 0, static_cast<int>(sz(adj[u]) - 1)}); // Residual edge with 0 capacity initially
    }
    ...
    ll _dfs_dinic(int u, int t, ll pushed) {
        ...
        if (u == t) return pushed; // Reached sink
        ...
        for (int& cid = ptr[u]; cid < sz(adj[u]); ++cid) {
            ...
            if (tr == 0) continue; // No flow sent through this path
    ...
    ll get_max_flow(int s, int t) {
        ...
        if (s == t) return INF; // Or 0, by convention

        ll total_flow = 0;
        while (_bfs_dinic(s, t)) { // While sink is reachable in level graph
...
inline vi build_suffix_array(const string& s_in) {
    ...
    if (n == 0) return {}; // Should not happen if s_in was non-empty due to sentinel
    if (n == 1 && s[0] == char(1)) return {0}; // Only sentinel was present
    ...
    for (int k = 4; k < 2 * n; k = k * 2) { // Cannot use simple f/cf due to k = k*2
...
inline vi build_lcp_array(const string& s, const vi& sa) {
    ...
    for (int i = 0; i < n; i++) {
        if (rank_inv[i] == n - 1) { // Suffix s[i...] is last in sorted order, no next suffix
            k = 0;
            continue;
        }
...
namespace NTT {
    ...
    inline ll power_ntt(ll base, ll exp) { // Marked inline
    ...
    inline void _transform(vll& a, bool invert) {
        ...
        for (int len = 2; len <= n; len <<= 1) { // Cannot use simple f/cf due to len <<= 1
            ll wlen = power_ntt(PRIMITIVE_ROOT, (MOD - 1) / len);
            if (invert) {
                wlen = M.inv(wlen);
            }
            for (int i = 0; i < n; i += len) { // Cannot use simple f/cf due to i += len
                ll w = 1;
                f(j, 0, len / 2) { // Used f macro
                    ll u = a[i + j];
        ...
        if (invert) {
            ll n_inv = M.inv(n);
            forV(a) { // Used forV macro, element is `e`
    ...
    inline vll multiply_polynomials(const vll& a_in, const vll& b_in) { // Renamed to avoid conflict with 'a' in forV
        if (a_in.empty() || b_in.empty()) return {};
        
        vll fa(all(a_in)), fb(all(b_in)); // Used all
        int result_deg = sz(a_in) + sz(b_in) - 2; // Used sz
        if (result_deg < 0) result_deg = 0; // Handle empty or constant product
        
        int n_ntt = 1; // Renamed n to n_ntt to avoid conflict
        while (n_ntt < sz(a_in) + sz(b_in) -1 ) n_ntt <<= 1; // Used sz. Smallest power of 2 >= result size
        if (sz(a_in) == 0 && sz(b_in) == 0) n_ntt = 0; // Used sz. Product of two empty polys is empty
        else if (n_ntt == 0 && (sz(a_in) > 0 || sz(b_in) > 0)) n_ntt=1; // Used sz. product of const and empty
        if (n_ntt==0 && (sz(a_in)>0 || sz(b_in)>0) && (sz(a_in)+sz(b_in)-1 > 0) ) n_ntt=1; // Used sz. for single element product, result is size 1
        ...
        n_ntt = 1;
        while(n_ntt < final_size) n_ntt <<=1;
        if (final_size == 0) n_ntt = 0; // if product is truly empty


        fa.resize(n_ntt); // Pad with zeros
        fb.resize(n_ntt);

        if (n_ntt > 0) { // Only transform if there are elements
            _transform(fa, false);
            _transform(fb, false);
            f(i, 0, n_ntt) { // Used f macro
...
struct ConvexHullTrickMinMono {
    ...
    void add_line(ll m, ll c) {
        ...
        while (sz(hull) >= 2) { // Used sz
            ...
            if (l1.intersect({m,c}) <= l1.intersect(l2)) { // Check with the incoming line {m,c}
                hull.pop_back();
            } else {
                break;
            }
        ...
        // Clamp query_ptr to be within valid bounds if it was pointing beyond the new end.
        // query_ptr should not exceed hull.size() - 1.
        // Since hull cannot be empty here (we just pushed), hull.size() - 1 is >= 0.
        query_ptr = std::min(query_ptr, (int)sz(hull) - 1); // Used sz
        // Also ensure query_ptr is not negative if it somehow became so (e.g. if hull was emptied and query_ptr wasn't reset)
        // Though with current logic, hull won't be empty here.
        if (query_ptr < 0 && !hull.empty()) query_ptr = 0; // Should not be needed if query_ptr is always >=0
        else if (hull.empty()) query_ptr = 0; // Safety for if hull could be empty
    }
...
vector<ResultType> mos_algorithm(
    ...
    if (n == 0) { // Handle empty array case: all queries (if any) likely get default/problem-specific empty result
        for(const auto& query : queries) {
            ...
            if (query.id < q_count) answers[query.id] = ResultType{}; 
        }
        return answers;
    }

    int block_size = sqrt(n);
    if (block_size == 0) block_size = 1; // Avoid division by zero if n is very small (e.g., 0, though handled)
    ...
    sort(all(queries), [&](const MoQuery& a, const MoQuery& b) {
        ...
        if (a.block % 2 == 0) { // Even block: sort by R ascending
            return a.r < b.r;
        }
...
struct Point {
    ...
    Point normalize() const {
        ld len = norm();
        if (std::abs(len) < EPS) return Point(0,0); // Or handle error, or return as is
...
inline vector<Point> convex_hull(vector<Point>& points) {
    ...
    if (n_pts <= 2) { // Hull is the points themselves if n_pts <= 2 (or empty if n=0)
    ...
    // Build upper hull
    f(i, 0, n_pts) { // Used f macro, ensure n_pts is used
        ...
        while (sz(upper_hull) >= 2 && orientation(upper_hull[sz(upper_hull)-2], upper_hull.back(), points[i]) != 2) { // Used sz
            upper_hull.pop_back();
        }
        upper_hull.pb(points[i]); // Used pb
    }

    // Build lower hull
    rf(i, n_pts, 0) { // Used rf macro, ensure n_pts is used
        ...
        // orientation == 1 is CW
        while (sz(lower_hull) >= 2 && orientation(lower_hull[sz(lower_hull)-2], lower_hull.back(), points[i]) != 1) { // Used sz
            lower_hull.pop_back();
        }
        lower_hull.pb(points[i]); // Used pb
    }
...
inline LineIntersectionStatus get_line_intersection(Point p1, Point q1, Point p2, Point q2, Point& intersection_point) {
    ...
    if (dir1.norm_sq() < EPS * EPS && dir2.norm_sq() < EPS * EPS) { // Both lines are points
        if ((p1 - p2).norm_sq() < EPS * EPS) { // Points are the same
    ...
    if (dir1.norm_sq() < EPS * EPS) { // Line 1 is a point
        if (orientation(p2, q2, p1) == 0) { // Point p1 lies on line p2q2
            intersection_point = p1;
            return LineIntersectionStatus::COLLINEAR;
        }
        return LineIntersectionStatus::LINES_ARE_POINTS;
    }
    if (dir2.norm_sq() < EPS * EPS) { // Line 2 is a point
        if (orientation(p1, q1, p2) == 0) { // Point p2 lies on line p1q1
    ...
    if (std::abs(det) < EPS) { // Lines are parallel or collinear
...
inline PointPair closest_pair_brute_force(const vector<Point>& points_sorted_x, int start_idx, int end_idx) {
    ...
    if (end_idx - start_idx + 1 < 2) return min_pair; // Not enough points

    cf(i, start_idx, end_idx) { // Used cf macro
        cf(j, i + 1, end_idx) { // Used cf macro
...
inline PointPair closest_pair_recursive(vector<Point>& points_sorted_x, vector<Point>& temp_strip_points, int start_idx, int end_idx) {
    ...
    if (n_pts_rec <= 3) { // Base case: brute force for 3 or fewer points
    ...
    if (min_dist_sq_so_far < EPS * EPS) { // If distance is effectively zero (e.g. duplicate points)
    ...
    cf(i, start_idx, end_idx) { // Used cf macro
        if (std::abs(points_sorted_x[i].x - mid_point.x) * std::abs(points_sorted_x[i].x - mid_point.x) < min_dist_sq_so_far) {
            temp_strip_points.pb(points_sorted_x[i]); // Used pb
        }
    }
    ...
    f(i, 0, sz(temp_strip_points)) { // Used f and sz macros
        f(j, i + 1, sz(temp_strip_points)) { // Used f and sz macros
             if (!((temp_strip_points[j].y - temp_strip_points[i].y) * (temp_strip_points[j].y - temp_strip_points[i].y) < min_dist_sq_so_far)) break; // Optimization
...
inline PointPair find_closest_pair(vector<Point>& points) {
    ...
    int n_pts_main = static_cast<int>(points.size()); // Renamed n to n_pts_main, used points.size()
    if (n_pts_main < 2) {
    ...
    // temp_strip_points.reserve(n_pts_main); // Original line with n (potentially ll from sz())
    temp_strip_points.reserve(static_cast<size_t>(n_pts_main)); // Correctly use size_t for reserve
